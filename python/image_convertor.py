...
SUPPORTED_EXTENSIONS = {".jp2", ".j2k", ".jpf", ".jpx", ".tif", ".tiff", ".jpg", ".jpeg", ".png", ".bmp"}
...
def read_downsampled_image(image_path: Path, factor: float, verbose: bool = False) -> Image.Image:
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=NotGeoreferencedWarning)
        ...
        with rasterio.open(image_path) as ds:
            ...
            verbose_print(f"   {src_w}x{src_h} → {out_w}x{out_h}", verbose)
...
def save_image(img: Image.Image, output_path: Path, output_format: str, jpeg_quality: int):
...
def infer_output_path(input_path: Path, output_path: Path | None, output_format: str) -> Path:
    if output_path is not None:
        return output_path

    return input_path.with_suffix(get_output_extension(output_format))
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert and optionally downsample a single raster image."
    )

    parser.add_argument("--input", required=True, help="Input image file")
    parser.add_argument("--output", default=None, help="Output image file (optional)")
    parser.add_argument(
        "--factor",
        type=float,
        default=1.0,
...
def main():
    ...
    input_path = sanitize_cli_path(args.input)
    output_path = sanitize_cli_path(args.output) if args.output else None
    ...
    output_path = infer_output_path(input_path, output_path, output_format)

    print("================================")
    print("🖼️ RASTER CONVERSION")
    print("================================")
